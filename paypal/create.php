...
/*
    ...
    $product_info = $mysqli->query("SELECT * FROM ProductInfo
                                  WHERE product_id = '".$mysqli->mysqli_real_escape_string($productid)."'");
   
    if(mysqli_num_rows($product_info) > 0){
        $row = mysqli_fetch_assoc($product_info);
...
$_SESSION['paymentId'] = $payment_id;
    ...
    $payment_info = $mysqli->query("INSERT INTO payment_records
                    (product_id,payment_id)
                    VALUES ('$productid','$payment_id'");
